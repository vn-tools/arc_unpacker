...
static void register_palettes(
    const std::string &arc_path, Pak2ImageDecoder &image_decoder)
{
...
void Pak2ArchiveDecoder::unpack_internal(File &arc_file, FileSaver &saver) const
{
    auto table = read_table(arc_file.io);
    register_palettes(arc_file.name, p->image_decoder);
