...
pix::Image PrtImageDecoder::decode_impl(io::File &input_file) const
{
    ...
    u16 bit_depth = input_file.stream.read_u16_le();
    u16 palette_offset = input_file.stream.read_u16_le();
    ...
    if (version == 0x66)
    {
        ...
        auto x = input_file.stream.read_u32_le();
        auto y = input_file.stream.read_u32_le();
        auto width2 = input_file.stream.read_u32_le();
        auto height2 = input_file.stream.read_u32_le();
