...
{
    const auto width
        = (file.io.seek(36).read_u16_le()
        - file.io.seek(32).read_u16_le() + 1) * 8;

    const auto height
        = file.io.seek(38).read_u16_le()
        - file.io.seek(34).read_u16_le() + 1;

    const bool horizontal = file.io.seek(40).read_u8() > 0;
    const auto base_color = file.io.seek(41).read_u8();
    const auto size_orig = file.io.seek(44).read_u32_le();
    const auto input = file.io.seek(48).read_to_eof();
    ...
    {
        const auto tmp = file.io.read_u8();
