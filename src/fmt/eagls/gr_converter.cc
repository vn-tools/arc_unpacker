...
bool GrConverter::is_recognized_internal(File &file) const
{
    return file.has_extension("gr");
}

std::unique_ptr<File> GrConverter::decode_internal(File &file) const
{
...
static auto dummy = fmt::Registry::add<GrConverter>("eagls/gr");
