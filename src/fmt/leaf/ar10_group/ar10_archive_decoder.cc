    ...
    Ar10ArchiveDecoder::read_meta_impl(File &arc_file) const
{
    ...
    const auto offset_to_data = arc_file.stream.read_u32_le();
    auto meta = std::make_unique<ArchiveMetaImpl>();
    meta->archive_key = arc_file.stream.read_u8();
    ...
    File &arc_file, const ArchiveMeta &m, const ArchiveEntry &e) const
{
    ...
    const auto data_size = arc_file.stream.read_u32_le();
    const auto key_size = arc_file.stream.read_u8() ^ meta->archive_key;
    const auto key = arc_file.stream.read(key_size);
    auto data = arc_file.stream.read(data_size);
