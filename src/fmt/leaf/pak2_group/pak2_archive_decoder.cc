...
bool Pak2ArchiveDecoder::is_recognized_impl(File &arc_file) const
{
    if (!arc_file.has_extension("pak"))
        return false;
    const auto file_count = arc_file.stream.seek(0x1C).read_u16_le();
    if (file_count < 2)
        return false;
    const auto value1 = arc_file.stream.seek(0x34).read_u32_le();
    const auto value2 = arc_file.stream.seek(0x3C).read_u32_le();
    for (const auto i : util::range(file_count))
    {
        if (arc_file.stream.seek(0x34 + i * 0x20).read_u32_le() != value1)
            return false;
        if (arc_file.stream.seek(0x3C + i * 0x20).read_u32_le() != value2)
            return false;
    }
    return true;
}
    ...
    Pak2ArchiveDecoder::read_meta_impl(File &arc_file) const
{
    ...
    arc_file.stream.seek(0x20);
    for (const auto i : util::range(file_count))
    {
        auto entry = std::make_unique<ArchiveEntryImpl>();
        arc_file.stream.skip(2);
        auto name = arc_file.stream.read(12);
        for (auto &c : name)
            c = (c << 4) | (c >> 4);
