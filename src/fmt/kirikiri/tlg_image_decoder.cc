...
static pix::Grid decode_proxy(int version, io::File &input_file);
...
static pix::Grid decode_tlg_0(io::File &input_file)
{
...
static pix::Grid decode_tlg_5(io::File &input_file)
{
    Tlg5Decoder decoder;
    return decoder.decode(input_file);
}

static pix::Grid decode_tlg_6(io::File &input_file)
{
    Tlg6Decoder decoder;
    return decoder.decode(input_file);
}
...
pix::Grid TlgImageDecoder::decode_impl(io::File &input_file) const
{
    int version = guess_version(input_file.stream);
    return decode_proxy(version, input_file);
}
