...
std::unique_ptr<fmt::ArchiveMeta> Hg3ImageArchiveDecoder::read_meta_impl(
...
fmt::NamingStrategy Hg3ImageArchiveDecoder::naming_strategy() const
{
    return NamingStrategy::Sibling;
}

static auto dummy = fmt::register_fmt<Hg3ImageArchiveDecoder>("cat-system/hg3");
