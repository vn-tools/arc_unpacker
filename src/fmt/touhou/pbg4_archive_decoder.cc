...
bool Pbg4ArchiveDecoder::is_recognized_internal(File &arc_file) const
{
    return arc_file.io.read(magic.size()) == magic;
}

void Pbg4ArchiveDecoder::unpack_internal(File &arc_file, FileSaver &saver) const
{
    ...
    for (auto &entry : table)
        saver.save(read_file(buf_io, *entry));
}

static auto dummy = fmt::Registry::add<Pbg4ArchiveDecoder>("th/pbg4");
