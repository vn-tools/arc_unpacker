...
namespace
{
    ...
    void decode(uint32_t decoder, char *buffer, size_t size)
    {
        ...
        auto func1 = decoders[func1_index];
        auto func2 = decoders[func2_index];
        ...
        size_t file_headers_start)
    {
        ...
        table_io.skip(4);
        int32_t parent_dir = table_io.read_u32_le();
        int32_t flags2 = table_io.read_u32_le();
    ...
    std::unique_ptr<File> read_file(IO &arc_io, const TableEntry &table_entry)
    {
        ...
        std::unique_ptr<char[]> buffer(new char[table_entry.size]);
        ...
        file->io.write(buffer.get(), table_entry.size);
        return file;
    }
}
